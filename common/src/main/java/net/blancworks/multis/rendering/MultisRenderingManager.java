...
public class MultisRenderingManager {
    public static final Identifier itemAtlasID = new Identifier("multis", "item_atlas");
    public static final BWAtlas itemAtlas = new BWAtlas(itemAtlasID, 16, 2, 2);

    public static ModelTransformation itemModelTransformation;
    public static ModelTransformation heldItemModelTransformation;

    private static CompletableFuture futureTask = CompletableFuture.completedFuture(null);

    public static void init() {
        MultisExpectPlatform.registerReloadListener("rendering", m -> {
            MinecraftClient.getInstance().getTextureManager().registerTexture(itemAtlasID, itemAtlas);
